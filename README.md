...
* Mutations that modify nodes should always include the updated version of that node in the result. This makes it easy for clients to maintain up-to-date state and tolerate eventual consistency (If a client updates a resource, and then requests it in a subsequent request, the server may provide a version of the resource that was cached before the update.).
